...
function obterLoginPorPerfil(perfil) {
    var rotas = {
        admin: "login.html",
        gerente: "login.html",
        organizacao: "login.html",
        organizador: "login.html",
        aluno: "login.html"
    };
...
function montarAreaUsuario(dados) {
    ...
    for (var i = 0; i < itensMenu.length; i++) {
        htmlItens +=
            '<li><a class="dropdown-item' + (itensMenu[i].ativo ? " active" : "") + '" href="' + CampusTrack.dom.escapeHtml(itensMenu[i].href) + '">' +
                CampusTrack.dom.escapeHtml(itensMenu[i].texto) +
            '</a></li>';
    }
    ...
    area.innerHTML =
        '<button type="button" class="btn btn-primary dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">' +
            CampusTrack.dom.escapeHtml(dados.nome) +
        '</button>' +
        '<ul class="dropdown-menu">' +
            '<li><h6 class="dropdown-header">' + CampusTrack.dom.escapeHtml(dados.perfil) + '</h6></li>' +
            htmlItens +
    ...
    botoes.forEach(function (botao) {
        ...
        if (botao.classList.contains("ct-btn-sair") || botao.textContent.trim().toLowerCase() === "sair") {
            ...
            botao.addEventListener("click", async function (event) {
                ...
                await CampusTrack.api.json("../../php/core/logout.php");
...
function valida_sessao() {
    ...
    sessaoValidacaoPromise = CampusTrack.api.json("../../php/core/sessao_status.php?perfil=" + encodeURIComponent(perfil))
