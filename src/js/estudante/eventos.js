...
async function carregarEventos() {
    ...
    try {
        const resposta = await CampusTrack.api.json(montarUrlEventos());

        if (resposta.status !== "ok") {
            lista.innerHTML = "";
            mostrarAlertaEventos(CampusTrack.resposta.mensagem(resposta), "danger");
            return;
        }
            ...
            html +=
                '<article class="col-12 col-md-6 col-xl-4">' +
                    '<div class="card ct-event-card h-100 shadow-sm">' +
                        '<div class="card-body">' +
                            badgeStatus +
                            '<h2 class="h5 card-title text-primary">' + CampusTrack.dom.escapeHtml(evento.nome) + '</h2>' +
                            '<p class="ct-event-meta mb-2">' + CampusTrack.dom.escapeHtml(evento.data_formatada || evento.data) + '</p>' +
                            '<p class="mb-1"><strong>Local:</strong> ' + CampusTrack.dom.escapeHtml(evento.nome_local) + '</p>' +
                            '<p class="mb-1"><strong>Instituicao:</strong> ' + CampusTrack.dom.escapeHtml(evento.nome_instituicao) + '</p>' +
                            '<p class="mb-0"><strong>Organizacao:</strong> ' + CampusTrack.dom.escapeHtml(evento.nome_organizacao) + '</p>' +
                            formFeedback +
                        '</div>' +
                    '</div>' +
                '</article>';
...
window.enviarFeedback = async function (event, idEvento) {
    ...
    try {
        const resposta = await CampusTrack.api.json("../../php/comentarios/adicionar.php", {
            method: "POST",
            body: data
        });
        ...
        if (resposta.status === "ok") {
            if (window.__alertaOriginal) {
                window.__alertaOriginal(CampusTrack.resposta.mensagem(resposta));
            } else {
