...
async function carregarInstituicoes() {
    var resposta = await CampusTrack.api.json("../../php/instituicoes/get.php");
    ...
    for (var i = 0; i < resposta.data.length; i++) {
        ...
        select.innerHTML += '<option value="' + CampusTrack.dom.escapeHtml(item.id_instituicao) + '">' + CampusTrack.dom.escapeHtml(item.nome) + '</option>';
    }
...
async function carregarLocais() {
    var resposta = await CampusTrack.api.json("../../php/locais/get.php?id_instituicao=" + encodeURIComponent(instituicaoAtual));
    ...
    for (var i = 0; i < resposta.data.length; i++) {
        ...
        L.marker([latitude, longitude])
            .bindPopup("<strong>" + CampusTrack.dom.escapeHtml(local.nome) + "</strong><br><span class='small'>" + CampusTrack.dom.escapeHtml(local.tipo) + "</span>")
            .addTo(camadaLocais);
...
async function carregarGrafo() {
    var resposta = await CampusTrack.api.json("../../php/mapa/grafo_get.php?id_instituicao=" + encodeURIComponent(instituicaoAtual));
...
function desenharNo(no) {
    ...
    marcador.bindPopup(
        "<strong>" + CampusTrack.dom.escapeHtml(no.nome) + "</strong><br>" +
        "<span class='small'>No #" + CampusTrack.dom.escapeHtml(id) + "</span>" +
...
function desenharAresta(aresta) {
    ...
    linha.bindPopup(
        "<strong>Conexao #" + CampusTrack.dom.escapeHtml(aresta.id_aresta) + "</strong><br>" +
        "<span class='small'>" + CampusTrack.dom.escapeHtml(origem.nome) + " -> " + CampusTrack.dom.escapeHtml(destino.nome) + "</span><br>" +
        "<span class='small'>Distancia: " + CampusTrack.dom.escapeHtml(aresta.distancia_metros) + " m</span><br>" +
        "<button type='button' class='btn btn-danger btn-sm mt-2' data-excluir-aresta='" + CampusTrack.dom.escapeHtml(aresta.id_aresta) + "'>Excluir</button>"
    );
...
async function salvarNo() {
    ...
    var url = id === "" ? "../../php/mapa/no_adicionar.php" : "../../php/mapa/no_alterar.php?id=" + encodeURIComponent(id);
    var resposta = await CampusTrack.api.json(url, { method: "POST", body: dados });
...
async function excluirNo() {
    ...
    var resposta = await CampusTrack.api.json("../../php/mapa/no_excluir.php?id=" + encodeURIComponent(id) + "&id_instituicao=" + encodeURIComponent(instituicaoAtual));
...
async function criarAresta() {
    ...
    var resposta = await CampusTrack.api.json("../../php/mapa/aresta_adicionar.php", {
        method: "POST",
        body: dados
    });
...
async function excluirAresta(id) {
    ...
    var resposta = await CampusTrack.api.json("../../php/mapa/aresta_excluir.php?id=" + encodeURIComponent(id) + "&id_instituicao=" + encodeURIComponent(instituicaoAtual));
