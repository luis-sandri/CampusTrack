...
async function carregarEventosPendentes() {
    ...
    const resposta = await CampusTrack.api.json("../../php/eventos/pendentes.php");
    ...
    if (resposta.data.length === 0) {
        ...
        for (var i = 0; i < resposta.data.length; i++) {
            ...
            html += '<tr>' +
                ...
                '<td>' + CampusTrack.dom.escapeHtml(e.nome_organizacao) + '</td>' +
                '<td>' +
...
async function alterarStatus(id, novo_status) {
    ...
    const resposta = await CampusTrack.api.json("../../php/eventos/alterar_status.php", {
        method: "POST",
        body: formData,
    });
